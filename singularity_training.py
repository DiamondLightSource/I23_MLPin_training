...
os.system(
    f"python {files['TF_RECORD_SCRIPT']} -x {os.path.join(paths['IMAGE_PATH'], 'train')} -l {files['LABELMAP']} -o {os.path.join(paths['ANNOTATION_PATH'], 'train.record')} "
)
os.system(
    f"python {files['TF_RECORD_SCRIPT']} -x {os.path.join(paths['IMAGE_PATH'], 'test')} -l {files['LABELMAP']} -o {os.path.join(paths['ANNOTATION_PATH'], 'test.record')} "
)

shutil.copy(
    (
        os.path.join(
            paths["PRETRAINED_MODEL_PATH"], PRETRAINED_MODEL_NAME, "pipeline.config"
        )
    ),
    (os.path.join(paths["CHECKPOINT_PATH"], "pipeline.config")),
)
...
command = (
    "python {} --model_dir={} --pipeline_config_path={} --num_train_steps=2000".format(
